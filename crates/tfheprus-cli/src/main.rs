...
use tfheprus_circuits::{
    ...
    SELECTOR_DIGEST_WIDTH, SHA3_DIGEST_WIDTH, SHA3_PBS_BSK_CHAIN_DOMAIN,
    SHA3_PBS_MASK_CHAIN_DOMAIN,
};
use tfheprus_core::{
...
use tfheprus_prover::{
    ...
    compact_accumulator_digest_public_values,
    ...
    deserialize_compact_pbs_keyswitch_proof,
    ...
    prove_glwe_keyswitch_private_key_digest, prove_mul_xai, prove_poly_mul,
    ...
    serialize_compact_pbs_keyswitch_proof,
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    match args.get(1).map(String::as_str) {
        ...
        Some("prove-glwe-keyswitch") => prove_glwe_keyswitch_demo(parse_preset_arg(&args)?)?,
        Some("prove-glwe-keyswitch-private-key-digest") => {
            prove_glwe_keyswitch_private_key_digest_demo(parse_preset_arg(&args)?)?
        }
        Some("bench-glwe-keyswitch-modes") => {
            bench_glwe_keyswitch_modes_demo(parse_preset_arg(&args)?)?
        }
        Some("prove-compact-root-keyswitch") => prove_compact_root_keyswitch_demo(
            parse_required_arg(&args, 2, "compact root artifact")?,
        )?,
        Some("prove-compact-root-keyswitch-recursive") => {
            prove_compact_root_keyswitch_recursive_demo(
                parse_required_arg(&args, 2, "compact root artifact")?,
                parse_required_arg(&args, 3, "final recursive artifact")?,
            )?
        }
        Some("verify-compact-root-keyswitch-recursive") => {
            verify_compact_root_keyswitch_recursive_demo(parse_required_arg(
                &args,
                2,
                "final recursive artifact",
            )?)?
        }
...
fn prove_glwe_keyswitch_demo(preset: ParamPreset) -> Result<(), Box<dyn Error>> {
    let (params, sk, instance) = glwe_keyswitch_demo_instance(preset);

    let prove_started = Instant::now();
    let proof = prove_glwe_keyswitch(&instance)?;
    let prove_time = prove_started.elapsed();
    let proof_bytes = glwe_keyswitch_proof_serialized_len(&proof)?;
    ...
    println!(
        ...
        instance.output.decrypt(&params, &sk.input_lwe)
    );

    Ok(())
}

fn prove_glwe_keyswitch_private_key_digest_demo(preset: ParamPreset) -> Result<(), Box<dyn Error>> {
    let (params, sk, instance) = glwe_keyswitch_demo_instance(preset);

    let prove_started = Instant::now();
    let proof = prove_glwe_keyswitch_private_key_digest(&instance)?;
    let prove_time = prove_started.elapsed();
    let proof_bytes = glwe_keyswitch_private_key_digest_proof_serialized_len(&proof)?;
    let verify_started = Instant::now();
...
fn bench_glwe_keyswitch_modes_demo(preset: ParamPreset) -> Result<(), Box<dyn Error>> {
    let (params, sk, instance) = glwe_keyswitch_demo_instance(preset);

    let public_prove_started = Instant::now();
    let public_proof = prove_glwe_keyswitch(&instance)?;
    let public_prove_time = public_prove_started.elapsed();
    let public_proof_bytes = glwe_keyswitch_proof_serialized_len(&public_proof)?;
    let public_verify_started = Instant::now();
    verify_glwe_keyswitch_proof(&instance, &public_proof)?;
    let public_verify_time = public_verify_started.elapsed();

    let private_prove_started = Instant::now();
    let private_proof = prove_glwe_keyswitch_private_key_digest(&instance)?;
    let private_prove_time = private_prove_started.elapsed();
    let private_proof_bytes =
        glwe_keyswitch_private_key_digest_proof_serialized_len(&private_proof)?;
    let private_verify_started = Instant::now();
    verify_glwe_keyswitch_private_key_digest_proof(&instance, &private_proof)?;
    let private_verify_time = private_verify_started.elapsed();

    let faster = if private_prove_time < public_prove_time {
        "private-key-digest"
    } else {
        "public-key"
    };
    println!(
        ...
        instance.private_inputs().len(),
        public_proof_bytes,
        public_prove_time.as_micros(),
        public_verify_time.as_micros(),
        private_proof.public_inputs.len(),
        instance.private_key_digest_private_inputs().len(),
        private_proof_bytes,
        private_prove_time.as_micros(),
        private_verify_time.as_micros(),
        faster,
        instance.output.decrypt(&params, &sk.input_lwe)
    );
...
fn glwe_keyswitch_demo_instance(preset: ParamPreset) -> (Params, SecretKey, GlweKeyswitchInstance) {
    glwe_keyswitch_instance_from_params(preset.params())
}

fn glwe_keyswitch_instance_from_params(
    params: Params,
) -> (Params, SecretKey, GlweKeyswitchInstance) {
    ...
    let instance = GlweKeyswitchInstance::new(params.clone(), accumulator, key_switch_key);
    (params, sk, instance)
}

fn prove_compact_root_keyswitch_demo(root_artifact_path: &str) -> Result<(), Box<dyn Error>> {
    let root_bytes = fs::read(root_artifact_path)?;
    let root_proof =
        deserialize_compact_aggregated_recursive_actual_pbs_chain_root_proof(&root_bytes)?;
    let root_summary = root_proof.chain_summary.clone();
    verify_compact_aggregated_recursive_actual_pbs_chain_root_summary_proof(
        &root_summary,
        &root_proof,
    )?;
    if root_summary.step_count != root_summary.params.lwe_dimension {
        return Err(format!(
            "compact root covers {} steps, expected full PBS dimension {}",
            root_summary.step_count, root_summary.params.lwe_dimension
        )
        .into());
    }

    let (params, sk, evaluation_key, input, test_polynomial) =
        actual_pbs_materials(root_summary.params.clone());
    let mut rng = ChaCha20Rng::seed_from_u64(301);
    let accumulator = blind_rotate_ntt(&params, &evaluation_key.to_ntt(), &input, &test_polynomial);
    let output_key = trivial_lwe_extraction_key(&params, &sk.input_lwe);
    let key_switch_key = GlweKeySwitchKey::generate(&params, &sk.glwe, &output_key, &mut rng);
    let instance = GlweKeyswitchInstance::new(params.clone(), accumulator, key_switch_key);
    let key_switch_public_inputs = instance.public_inputs();
    let accumulator_field_count = (params.glwe_dimension + 1) * params.polynomial_size;
    let key_switch_accumulator_digest = compact_accumulator_digest_public_values(
        &key_switch_public_inputs[..accumulator_field_count],
    );
    let root_accumulator_digest = root_summary.output_accumulator_digest()?;
    if key_switch_accumulator_digest != root_accumulator_digest {
        return Err(
            "compact root output accumulator digest does not match key-switch input".into(),
        );
    }

    let prove_started = Instant::now();
    let key_switch_proof = prove_glwe_keyswitch(&instance)?;
    let prove_time = prove_started.elapsed();
    let verify_started = Instant::now();
    verify_glwe_keyswitch_proof(&instance, &key_switch_proof)?;
    let verify_time = verify_started.elapsed();

    println!(
        "compact-root plus glwe-keyswitch proofs verified: root_artifact={}, root_artifact_bytes={}, params_n={}, total_steps={}, root_public_inputs={}, key_switch_public_inputs={}, key_switch_private_inputs={}, key_switch_prove_ms={}, key_switch_prove_us={}, key_switch_verify_ms={}, key_switch_verify_us={}, keyswitched_output_message={}",
        root_artifact_path,
        root_bytes.len(),
        params.lwe_dimension,
        root_summary.step_count,
        root_proof.root.public_input_count(),
        key_switch_proof.public_inputs.len(),
        instance.private_inputs().len(),
        prove_time.as_millis(),
        prove_time.as_micros(),
        verify_time.as_millis(),
...
fn prove_compact_root_keyswitch_recursive_demo(
    root_artifact_path: &str,
    ...
    let (params, sk, instance) = glwe_keyswitch_instance_from_params(root_summary.params.clone());
    let key_switch_public_inputs = instance.public_inputs();
    let accumulator_field_count = (params.glwe_dimension + 1) * params.polynomial_size;
    let key_switch_accumulator_digest = compact_accumulator_digest_public_values(
        &key_switch_public_inputs[..accumulator_field_count],
    );
    if key_switch_accumulator_digest != root_summary.output_accumulator_digest()? {
        return Err(
            "compact root output accumulator digest does not match key-switch input".into(),
        );
    }
    ...
    let prove_started = Instant::now();
    let final_proof = prove_compact_pbs_root_keyswitch_recursive(&root_proof, &instance)?;
    let prove_time = prove_started.elapsed();
    let verify_started = Instant::now();
    verify_compact_pbs_keyswitch_proof(&final_proof)?;
    let verify_time = verify_started.elapsed();
    let final_bytes = serialize_compact_pbs_keyswitch_proof(&final_proof)?;
    if let Some(parent) = Path::new(final_artifact_path).parent() {
        if !parent.as_os_str().is_empty() {
            fs::create_dir_all(parent)?;
        }
    }
    fs::write(final_artifact_path, &final_bytes)?;
    let decoded = deserialize_compact_pbs_keyswitch_proof(&final_bytes)?;
    verify_compact_pbs_keyswitch_proof(&decoded)?;

    println!(
        "compact-root plus glwe-keyswitch recursive proof verified: root_artifact={}, root_artifact_bytes={}, final_artifact={}, final_artifact_bytes={}, params_n={}, total_steps={}, final_public_inputs={}, recursive_prove_ms={}, recursive_prove_us={}, recursive_verify_ms={}, recursive_verify_us={}, keyswitched_output_message={}",
        root_artifact_path,
        root_bytes.len(),
        final_artifact_path,
        final_bytes.len(),
        params.lwe_dimension,
        root_summary.step_count,
        final_proof.public_input_count(),
        prove_time.as_millis(),
        prove_time.as_micros(),
        verify_time.as_millis(),
        verify_time.as_micros(),
...
fn verify_compact_root_keyswitch_recursive_demo(
    final_artifact_path: &str,
) -> Result<(), Box<dyn Error>> {
    let bytes = fs::read(final_artifact_path)?;
    let proof = deserialize_compact_pbs_keyswitch_proof(&bytes)?;
    let verify_started = Instant::now();
    verify_compact_pbs_keyswitch_proof(&proof)?;
    let verify_time = verify_started.elapsed();

    println!(
        "compact-root plus glwe-keyswitch recursive artifact verified: artifact={}, artifact_bytes={}, params_n={}, total_steps={}, final_public_inputs={}, output_lwe_fields={}, verify_ms={}, verify_us={}",
        final_artifact_path,
        bytes.len(),
        proof.summary.pbs.params.lwe_dimension,
        proof.summary.pbs.step_count,
        proof.public_input_count(),
        proof.summary.output_lwe.len(),
        verify_time.as_millis(),
        verify_time.as_micros()
    );
...
fn run_actual_pbs_native_demo(preset: ParamPreset) {
    ...
    let (input, test_polynomial) = actual_pbs_input(&params, &sk, &mut rng);
...
fn actual_pbs_input<R: rand::RngCore + ?Sized>(
    params: &Params,
    sk: &SecretKey,
    rng: &mut R,
) -> (LweCiphertext, TestPolynomial) {
    let input_message = 1;
